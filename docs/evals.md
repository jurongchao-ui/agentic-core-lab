...
python3 -m evalops.judge_registry validate --input data/eval-golden.json --json
```
...
python3 -m evalops.dataset --backend sqlite --path data/events.db --output data/eval-dataset.json
```
...
python3 -m evalops.sampling --input data/eval-dataset.json --output data/eval-review-queue.json
```
...
python3 -m evalops.harness --cases data/eval-dataset.json --json
```

审核 dataset 草稿并要求只跑已审核 case:

```bash
python3 -m evalops.review list --input data/eval-dataset.json
...
```bash
python3 -m evalops.review_store init --path data/reviews.db
python3 -m evalops.review_store import --path data/reviews.db --input data/eval-golden.json
python3 -m evalops.review_store list --path data/reviews.db
python3 -m evalops.review_store query --path data/reviews.db --reviewer local --limit 20 --offset 0 --json
python3 -m evalops.review_store state --path data/reviews.db --input data/eval-golden.json --json
```
...
python3 -m evalops.diff --base data/eval-base.json --candidate data/eval-candidate.json
python3 -m evalops.diff --base data/eval-base.json --candidate data/eval-candidate.json --json
...
python3 -m evalops.history list --history data/eval-history.jsonl --json
```
...
```bash
TOKEN=$(python3 -m evalops.governance.auth_tokens create \
