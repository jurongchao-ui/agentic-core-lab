...
项目里近 40 个模块看着多,其实反复在用**同样的三招**。认出招式,就不用死记每个模块。
...
- **`policies/safety_review.py`** — 安全判定为 `review`(需人工审核)时,把请求入队(内存/JSONL)留证。
  *必要性*:`review` 既不是 allow 也不是 refuse——得有地方把"待人工看"的请求留下证据,而不是丢掉。
...
- **`tools/middleware.py`** — 工具调用的横切管道(审批/成本/超时/重试/幂等/输出脱敏/OTel span)。
  *必要性*:这些是"每次工具调用前后都要做"的事,不该散落在业务里。是生产级横切逻辑的统一挂载点。幂等缓存 write 工具结果、`ToolOutputSafetyMiddleware` 连工具*输出*里的敏感值也抹掉。
...
- **`observability/event_payloads.py`** — 事件 payload 的类型级 schema + 版本化迁移。
  *必要性*:让不同事件类型的 payload 有明确字段;并能把旧格式事件(扁平/缺字段)在读取时迁移到当前 schema,老日志不作废。
